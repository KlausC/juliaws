...
# "generic copy format for all immutable and type objects" 
function copy1(x::Any)
	args = []
	for field in fieldnames(x)
		push!(args, getfield(x, field))
	end
	(typeof(x))(args...)
end
...
# "make a copy if mutable data type with fields"
...
# "make a copy, modifying some fields"
@generated function copy2(x::Any)
...
@generated function copy3(x::Any, dic::Dict)
	ex = :($x()) # constructor call without arguments
	exf = ex.args
	for f in fieldnames(x)
		push!(exf, :(getval(dic, $(QuoteNode(f)), x.$f)))
	end
	ex
end

function copy1(x::Any, dic::Dict)
	args = []
	for field in fieldnames(x)
		fun = get(dic, Symbol(field), identity)
		push!(args, fun(getfield(x, field)))
	end
	(typeof(x))(args...)
end
