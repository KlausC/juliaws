...
function Keccak_p(b::Integer, nr::Integer)
  ...
  const l = Int64(log2(w)) # throws InexactError if log2(w) is not integera
  const maskw = (UInt64(1) << w ) - 1
...
function rotl(u::UInt64, offset::Int)
  mm = maskw
  ww = w
  offset = (offset + ww) % ww	# opt 14/336
  wo = offset - ww
  u1 = u << offset
  u2 = u >> wo
  u1 = ( u1 | u2 )
...
#	copy!(As, A); return
  ...
  @inbounds for k = 1:5
	  # cc = ($)(A[:,k]...) # opt 41/336
	  cc = ($)(A[1,k], A[2,k], A[3,k], A[4,k], A[5,k]) # opt 0.4/224 !!!
	  C[k] = cc
	  Cs[k] = rotl(cc, 1) # opt 40/336
  end
  @inbounds for k = 1:5
	km1, kp1 = (k + 3) % 5, (k + 5) % 5
	dd = C[km1+1] $ Cs[kp1+1]
	@inbounds for j = 1:5
      As[j,k] = A[j,k] $ dd # opt +26/336
	end
  end
  nothing
end
...
#	copy!(As, A); return
  roa = RhoOffset
  @inbounds for x = 1:5, y = 1:5
    ro = roa[x,y]
    As[x,y] = rotl(A[x,y], ro) # opt 193/336
  end
  #map!(rotl, As, A, roa)
  nothing
end
...
#	copy!(As, A); return
  for x = 0:4
    for y = 0:4
		@inbounds As[y+1,x+1] = A[x+1,(3 * y + x + 5) % 5+1]
    end
  end
  nothing
end
...
#	copy!(As, A); return
  @inbounds for x = 0:4
    for y = 0:4
		As[y+1,x+1] = A[y+1,x+1] $ (~(A[y+1,(x+6)%5+1]) & A[y+1,(x+7)%5+1])
    end
  end
  nothing
end
